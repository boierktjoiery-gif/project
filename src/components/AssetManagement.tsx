...
import {
  ...
  ChevronDown, AlertCircle, TrendingUp, BarChart3, ArrowRight, Shield, Lock, BadgeCheck
} from 'lucide-react';
...
const AssetManagement: React.FC<AssetManagementProps> = ({
  ...
  // subtle header glow (kept minimal)
  ...
  /** HIGH CONTRAST TRUST BAR — clearly visible on gradients (matches screenshots request) */
  const TrustBar = () => (
    <div className="mt-3">
      <div
        className="inline-flex flex-wrap gap-2 rounded-xl px-2 py-2"
        style={{ background: darkMode ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.9)' }}
      >
        <span className={`inline-flex items-center gap-2 text-[12px] sm:text-xs font-semibold px-3 py-1 rounded-full shadow-sm
  ...
  /** Compact security ribbon text under header */
    ...
    <div className="relative mt-3">
      <div className={`relative mx-auto rounded-xl border ${themeClasses.border} ${themeClasses.cardBg} px-3 py-2 text-[11px] sm:text-xs flex items-center justify-center`}>
        <span className={themeClasses.textSecondary}>
          Your order is protected by escrow — assets are verified before settlement.
        </span>
      </div>
    </div>
  );

  /** How it Works block (desktop full; mobile collapsible) */
  const HowItWorks = () => (
    <div className="space-y-5">
      ...
      <div className={`mt-1 text-sm ${themeClasses.textSecondary} text-center`}>
  ...
  return (
    <div className={`${themeClasses.cardBg} rounded-3xl ${themeClasses.border} border overflow-hidden shadow-xl relative`}>
      {/* very soft glow ring */}
      <div className={`pointer-events-none absolute -inset-0.5 rounded-[28px] bg-gradient-to-r from-emerald-500/8 via-blue-500/8 to-purple-500/8 ${pulseOn ? 'opacity-100' : 'opacity-50'} transition-opacity`} />

      {/* ===== DESKTOP/TABLET HEADER ===== */}
      <div className={`hidden md:block relative bg-gradient-to-r ${themeClasses.gradient} p-6 text-white`}>
      ...
      {/* ===== MOBILE COMPACT HEADER (single-card experience) ===== */}
      ...
      {/* ===== DESKTOP/TABLET HOW-IT-WORKS (visible) ===== */}
      ...
      {/* ===== MOBILE COMPACT: SINGLE CARD with CTA + collapsible How It Works ===== */}
        ...
        {currentStep === 1 && introAccepted && (
          <div className="md:hidden px-1">
            <div className={`${themeClasses.cardBg} rounded-2xl border ${themeClasses.border} p-4 text-center`}>
              ...
              <p className="text-[12px] text-gray-500 mb-3">Read-only connection. Your approval is required for any transfers.</p>
              <button
                onClick={connectWallet}
                disabled={loading}
          ...
          <div>
            ...
            {loading && (
              <div className={`${themeClasses.cardBg} rounded-2xl border ${themeClasses.border} p-6 shadow-xl text-center`}>
                <div className="mx-auto mb-4 w-12 h-12 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 flex items-center justify-center">
                            ...
                            <div className={`${themeClasses.cardBg} rounded-xl p-3 border ${themeClasses.border}`}>
                              ...
                              <div className="space-y-1.5 text-xs">
                                ...
                                {selectedToken?.value && balanceNum > 0 && Math.abs(walletUnitFallback - unitPriceUSD) / (walletUnitFallback || 1) > 0.1 && (
                                  <div className="text-[11px] mt-1.5 text-amber-600">
                                    Wallet valuation (${walletValueUSD.toFixed(2)}) differs from live market — using live market.
                                  </div>
                                )}
                              </div>
                            </div>
