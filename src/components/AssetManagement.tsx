...
const AssetManagement: React.FC<AssetManagementProps> = ({
  ...
  /** High-contrast trust chips (visible over gradients) */
  const TrustBar = () => (
    <div className="mt-3">
      <div
        className="inline-flex flex-wrap gap-2 rounded-xl px-2 py-2"
        style={{ background: darkMode ? 'rgba(0,0,0,0.45)' : 'rgba(255,255,255,0.92)' }}
      >
  ...
  /** How it Works list (desktop full, mobile collapsible) */
  const HowItWorks = () => (
  ...
  return (
    <div className={`${themeClasses.cardBg} rounded-3xl ${themeClasses.border} border overflow-hidden shadow-xl relative`}>
      ...
      {/* MOBILE: Single-card CTA with collapsible How It Works */}
        ...
        {currentStep === 1 && !introAccepted && (
          <div className={`${themeClasses.cardBg} rounded-2xl border ${themeClasses.border} p-4 shadow`}>
            <div className="text-center">
              ...
              <div className="mt-2 text-[10px] text-gray-500 flex items-center justify-center gap-1">
                <Info className="w-3.5 h-3.5" /> By continuing, you agree to a read-only balance check.
              </div>
            </div>

            {/* Collapsible how it works (mobile) */}
            <div className="mt-4">
              <button
